...
parser.add_argument('--val_result_dir', help='directory to store val results')
...
if opt.type == 'crnn':
    ...
    model = crnn.CRNN(32, 100, 1, nclass, 256)
...
def test_batch(lmdbData):
    ...
    for i in range(nSamples):
        ...
        for i in lab.strip():
            if i not in vocab:
                print("NEW:: label character not in lexicon", i)
                flag = 1
                non_chars[i] = 1
        if flag == 1:
            non += 1
        ...
        if flag == 0:
            ...
            # print("NEW:: all label characters in lexicon", pred, gt)
    ...
    corr_wrr = nwrr / float(curr) * 100
    corr_crr = norm_ED / float(curr) * 100

    print("Only CLEANED IMAGES: {0}".format(curr))

    print("Word Recognition Rate :  " + str(corr_wrr))
    print("Character Recognition Rate :  " + str(corr_crr))
...
with open(os.path.join(opt.val_result_dir, f"{opt.type}_val_synthetic_{opt.lan}.txt"), "w") as f:
    f.write("FINAL RESULTS\n")
    f.write("Word Recognition Rate :" + str(wrr) + "\n")
    f.write("Character Recognition Rate :" + str(crr) + "\n")
