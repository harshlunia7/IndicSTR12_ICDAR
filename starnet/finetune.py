...
crnn = crnn.CRNN(opt.imgH, opt.imgW, nc, nclass, opt.nh)
...
def val(net, dataset, criterion, max_iter=100):
    ...
    print("max_iter val dataloader", max_iter)
    for i in range(max_iter):
        data = next(val_iter)

        i += 1
        cpu_images, cpu_texts = data
        # print("data shape val dataloader", cpu_images.size())
...
for epoch in range(opt.nepoch):
    ...
    while i < len(train_loader):
        ...
        if i % opt.valInterval == 0:
            ...
            with open(os.path.join(opt.expr_dir, f"val_values_{opt.lan}_finetune.txt"), "a") as f_val:
                print(f"Val entry for epoch: {epoch} - Accuracy: {acc}; loss: {lossval}; crr: {crr}", file = f_val)
